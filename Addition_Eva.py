...
if __name__ == "__main__":
    ...
    ### training data testing data added data
    Data_Size = 150
    Test_Size = 500
    Add_Size = 150
    ...
    add_data, add_lab_org, add_lab = x_train[300:300+Add_Size, :], y_train[300:300+Add_Size], y_train_enc[300:300+Add_Size,:]

    addition_evaluation = addition_eva(train_data, train_lab_org, x_test, y_test,
                                       add_data, add_lab_org, config, save_path, SEED)

    #%% generate random deletion subsets

    small_size = 5
    subset_index, size_record = addition_evaluation._create_random_subsets(small_size)
    [subset_index, size_record] = pickle.load(open(save_path + '/subset_size.data', 'rb'))

    #%% baseline result
    # addition_evaluation._baseline_result(subset_index)
    [subset_index, true_loss_change] =pickle.load(open(save_path + '/baseline_add_subset_losschange.data', 'rb'))
    ...
    addition_evaluation._deepset_result(subset_index, Util_deep_mod_load)
    ...
    for i in range(rep):
        ...
        addition_evaluation._influence_result(subset_index)
        [inf_all, inf_avg] = pickle.load(open(save_path + '/inlfu_add_time_avg.data', 'rb'))
        influ_time_all.append(inf_avg)
    addition_evaluation._influence_result(subset_index)

    #%% deeppara (parameter model)
    ...
    Util_deep_mod_load = Para_KKT_deepset(mod_config['dim'], mod_config['class_number'],
                                          Util_dim, deep_config, mod_config, model=deepmodel)
    addition_evaluation._deeppara_result(subset_index, Util_deep_mod_load)

    #%% results evaluation

    # baseline
    [para_true, loss_true, subset_size] = pickle.load(open(save_path + '/baseline_add_para_loss_size.data', 'rb'))

    # deepset
    [ para_kkt, loss_kkt, subset_kkt] = pickle.load(open(save_path + '/KKTUTL_add_para_loss_size.data', 'rb'))

    # influence
    [para_inf, loss_inf, subset_inf] = pickle.load(open(save_path + '/inlfu_add_para_loss_size.data', 'rb'))
    # deeppara
    [para_dp, loss_dp, subset_dp] = pickle.load(open(save_path + '/PARA_add_para_loss_size.data', 'rb'))

    para_dist= para_comp_add_fig(para_true, para_kkt,para_inf, para_dp, subset_size,config['dataset_name'],
                                 mod_config['base_model'],Data_Size, Add_Size)
    para_dist.savefig(save_path + '/para_dist_add_inf_C1_LR.png', bbox_inches = 'tight')
    para_dist.show()

    subset_size = np.array(subset_size) + Data_Size
    fig_mnist = all_loss_fig(loss_true, loss_kkt, loss_inf, loss_dp, subset_size)
    fig_mnist.savefig(save_path + '/Loss_compare_add_C1_LR.png', bbox_inches = 'tight')
    fig_mnist.show()

    [exact_all, exact_avg, exact_time_all] = pickle.load(open(save_path + '/baseline_add_time_avg.data', 'rb'))
    [kkt_all, kkt_avg, kkt_time_all] = pickle.load(open(save_path + '/KKTUTL_add_time_avg.data', 'rb'))
    [dp_all, dp_avg, dp_time_all] = pickle.load(open(save_path + '/PARA_add_time_avg.data', 'rb'))
    ...
    SEED = 33
    X_train, X_test, y_train, y_test = train_test_split(X_std, y, test_size=0.15, random_state=SEED)
    ...
    Train_size = 67
    Add_Size = X_train.shape[0] - Train_size
    X_add, y_add = X_train[Train_size:], y_train[Train_size:]
    addition_evaluation = addition_eva(X_train[:Train_size], y_train[:Train_size],
                                       X_test, y_test,
                                       X_add, y_add, config, save_path, SEED)
    #%% generate random addition subsets
    small_size = 1
    subset_index, size_record = addition_evaluation._create_random_subsets(small_size)
    [subset_index, size_record] = pickle.load(open(save_path + '/subset_size.data', 'rb'))
    ...
    subset_size = np.array(subset_size) + Train_size
    fig_iris = all_loss_fig(loss_true, loss_kkt, loss_inf, loss_dp, subset_size)
    fig_iris.savefig(save_path + '/Loss_compare_add_C1_LR.png', bbox_inches = 'tight')
    fig_iris.show()
    ...
    X_add, y_add = X_train[300:300 + Add_Size, :], y_train[300:300+Add_Size]
    X_train, y_train = X_train[:Data_size], y_train[:Data_size]
    config['dim'] = X_train.shape[1]
    SEED = 43

    addition_evaluation = addition_eva(X_train, y_train,
                                       X_test, y_test,
                                       X_add, y_add, config, save_path, SEED)
    ...
    fig_spam.savefig(save_path + '/Loss_compare_add_C1_LR.png', bbox_inches = 'tight')
    fig_spam.show()
    ...
    Data_size = 150
    Add_Size = 150
    X_add, y_add = X_train_all[300:300+Add_Size, :], y_train_all[300:300+Add_Size]
    ...
    fig_higgs.savefig(save_path + '/Loss_compare_add_C1_LR.png', bbox_inches = 'tight')
    fig_higgs.show()
