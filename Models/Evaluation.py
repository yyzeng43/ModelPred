# randomly generate addition / deletion size and record the computation time
...
import torch
from Models.UtilityModel import MINST_Utility,Iris_Utility
from influence.logistic_reg import multi_log, bin_log
import pandas as pd
...
def para_comp_fig(para_true, para_kkt, para_inf, para_dp, subset_size, dataset, base_model, n_train):

    dist_kkt, mean_kkt, std_kkt = [], [], []
    dist_inf, mean_inf, std_inf = [], [], []
    dist_dp, mean_dp, std_dp = [], [], []
    ...
    # sorted_delta = group_by(np.array(dist_delta), np.array(subset_size))
    ...
    fig, ax = plt.subplots(figsize=(10,7))#figsize=(20, 30)
    font = {'family':'serif','serif':['Times'], 'weight': 'normal', 'size': 30}
    plt.rc('font', **font)
    plt.plot(sizes, mean_kkt, label='OptLearn', color='orange', marker='o', markersize=10)
    # plt.plot(sizes, mean_delta, label='Deltagrad', color='blue', marker='o', markersize=10)
    ...
    # plt.fill_between(sizes,mean_delta+std_delta, mean_delta-std_delta, alpha=0.2, color='blue')
    ...
    plt.ylabel('Distance',fontsize=30)#, fontsize = 15
    plt.xlabel('Size of Remaining Training Subset', fontsize=30)  # , fontsize = 15
...
def para_comp_add_fig(para_true, para_kkt, para_inf,para_dp, subset_size, dataset, base_model, n_train, n_add):
    ...
    ax.xaxis.set_major_formatter(mtick.PercentFormatter())
    plt.ylabel('Distance', fontsize=30)#, fontsize = 15
    plt.xlabel('Size of Added Training Subset',fontsize=30)#, fontsize = 15
    plt.yticks(fontsize = 30)
    plt.xticks(fontsize=30)
    plt.legend(loc='upper right', prop = {'size': 30})
    return fig
...
def time_comparison(base_time, kkt_time, influ_time, dp_time):
    ...
    mean_dp = np.mean(np.array(dp_time))
    std_dp = np.std(np.array(dp_time))

    print('Solver time mean: {}, Std: {}'.format(mean_base,std_base))
    print('KKT time mean: {}, Std: {}'.format(mean_kkt, std_kkt))
    print('Para time mean: {}, Std: {}'.format(mean_dp, std_dp))
    print('Influence time mean: {}, Std: {}'.format(mean_inf, std_inf))


#%%
def all_loss_fig(loss_true, loss_kkt, loss_inf, loss_para, subset_size):
    ...
    mse_dp, nrmse_dp, spear_dp = [], [], []
    for rep in range(10):
        mse_dp.append(mean_squared_error(np.array(loss_true).reshape(10, -1)[rep], np.array(loss_para).reshape(10, -1)[rep]))
        nrmse_dp.append(np.sqrt(mean_squared_error
                                 (np.array(loss_true).reshape(10, -1)[rep] , np.array(loss_para).reshape(10, -1)[rep]))
                         / (np.max(np.array(loss_true).reshape(10, -1)[rep])- np.min(np.array(loss_true).reshape(10, -1)[rep])))
        spear_dp.append(spearmanr(np.array(loss_true).reshape(10, -1)[rep], np.array(loss_para).reshape(10, -1)[rep])[0])

    print('Para utility testing loss MSE: {}, NRMSE: {}, NRMSE std: {}'.format(np.mean(mse_dp), np.mean(nrmse_dp),
                                                                                      np.std(nrmse_dp)))
    print('loss spearman: {}, std: {}'.format(np.mean(spear_dp), np.std(spear_dp)))
...
class deletion_eva(object):
    ...
    def _deepset_result(self, subset_index, deepmodel):
        ...
        for subset in subset_index:
            ...
            if len(np.unique(y_use)) == self.config['class_number']:
                ...
                # start_time2 = time.time()
                # print(start_time2-start_time1, deepmodel.time)
    ...
    def _deeppara_result(self, subset_index, deepmodel):
        self.deeppara_result = {}
        self.deeppara_result['loss'] = []
        self.deeppara_result['parameters'] = []
        self.deeppara_result['size'] = []
        self.deeppara_result['time'] = []
        # one_hot_all = []
        ...
        for i in range(10):
            ...
            _ = deepmodel.predict(self.X_train, self.y_train, X_test_feature)
            end_time = time.time()
            self.deeppara_result['time'].append((end_time-start_time)/rep_length)
        ...
        for subset in subset_index:
            ...
            if len(np.unique(y_use)) == self.config['class_number']:
                ...
                estimation = estimation_all[recorder-1]
                loss = Utility_model._deep_loss(estimation.reshape(1, -1))
                self.deeppara_result['loss'].append(loss)
                self.deeppara_result['parameters'].append(estimation.flatten())
                self.deeppara_result['size'].append(len(subset))

        # end_time = time.time()
        avg_time = (end_time - start_time)/len(self.deeppara_result['size'])
        pickle.dump([end_time-start_time, avg_time, self.deeppara_result['time']],
                    open(self.PATH+'/PARA_del_time_avg.data', 'wb'))
        print('Deepset Time: {}'.format(end_time - start_time))
        pickle.dump([self.deeppara_result['parameters'], self.deeppara_result['loss'], self.deeppara_result['size']],
                open(self.PATH + '/PARA_del_para_loss_size.data', 'wb'))
    ...
    def _influence_result(self, subset_index):
        ...
        if self.config['class_number'] > 2:
            ...
            inf_para_change_loo = influ_log.get_single_para_loo(model, self.X_train.numpy(), self.y_train_enc)
        else:
            Utility_model = Iris_Utility(self.X_test, self.y_test, self.config, self.seed)
            influ_log = bin_log(self.C)
...
class addition_eva(object):
    ...
    def _deepset_result(self, subset_index, deepmodel):
        ...
        for subset in subset_index:
            ...
            if len(np.unique(y_use)) == self.config['class_number']:
                ...
                # start_time2 = time.time()
                # self.deepset_result['time'].append(start_time2-start_time1)
                recorder +=1
    ...
    def _deeppara_result(self, subset_index, deepmodel):
        ...
        for subset in subset_index:
            ...
            if len(np.unique(y_use)) == self.config['class_number']:
                ...
                #                                y_use, one_hot.reshape(1, -1))
                # start_time2 = time.time()
                # self.deeppara_result['time'].append(start_time2-start_time1)
                recorder +=1
                estimation = np.array(estimation_all)[recorder-1].flatten()
                # print(start_time2 - start_time1)
                loss = Utility_model._deep_loss(estimation.reshape(1, -1))
        ...
        avg_time = (end_time - start_time) / len(self.deeppara_result['size'])
        pickle.dump([end_time - start_time, avg_time, self.deeppara_result['time']],
                    open(self.PATH + '/PARA_add_time_avg.data', 'wb'))
        pickle.dump([self.deeppara_result['parameters'], self.deeppara_result['loss'], self.deeppara_result['size']],
                open(self.PATH + '/PARA_add_para_loss_size.data', 'wb'))
    ...
    def _influence_result(self, subset_index):
        ...
        if self.config['class_number'] > 2:
            Utility_model = MINST_Utility(self.X_test, self.y_test, self.y_test_enc, self.config, self.seed)
            influ_log = multi_log(self.C)
            inf_loss_change_loo = influ_log.get_add_loss_all(model, self.X_train.numpy(), self.y_train_enc,
                                                             self.X_add.numpy(), self.y_add_enc,
                                                              self.X_test.numpy(), self.y_test_enc)
            inf_para_change_loo = influ_log.get_single_para_add(model, self.X_train.numpy(), self.y_train_enc,
                                                                self.X_add.numpy(), self.y_add_enc)
