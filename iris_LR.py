# get training samples and train DNN
...
if __name__ == "__main__":
    ...
    NUM_SAMPLE = 10000
    Perm_NUM = 300   # 15 * 67 = 1005
    LR_data = LR_multiclass_preparation(config)
    if PERMSET == 'Rand':
        LR_data.random_sampling_generation(X_train, y_train, NUM_SAMPLE, 3, X_train.shape[0], SEED)
    elif PERMSET == 'Perm':
        LR_data.permutation_generation(X_train, y_train, Perm_NUM) # get the logistic regression model
    ...
    train_loss, valid_loss = Util_deep_mod.fit(X_train, y_train, train_set, valid_set,
                                               1 / config['lambda'], n_epoch, kkt_lambda=1)
    model_savename = result_path + '/Iris_LR_{}_{}_Del0_KKT_Utl_{}_L1L2_{}_{}_C_{}.state_dict'.\
    ...
    from Models.UtilityModel import Iris_Utility
    ...
    Util_deep_mod = Para_KKT_deepset(num_features, 1, Util_dim, deep_config, config)
    Util_deep_mod.fit(X_train, y_train,  train_set, valid_set,
                      1 / config['lambda'], n_epoch, kkt_lambda=0)
    model_savename = result_path + \
                     '/Iris_LR_Perm_regressor_Extr_Del0_Utl_15_NepochFalse_L1L2_128_128_C_1.0.state_dict'
    sys.stdout.flush()
    torch.save(Util_deep_mod.model.state_dict(), model_savename)
